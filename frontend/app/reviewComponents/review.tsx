...
const getOpenStatusWithTimes = (hoursMap: Record<string, string>) => {
  ...
  const normalizedTodayHours = todayHours
    .replace(/\u2013|\u2011/g, "-")
    .replace(/\s+/g, " ")
    .trim();
  const [openTime, closeTime] = normalizedTodayHours
    .split("-")
    .map((time) => time.trim());
...
function findNextOpening(
  ...
  for (let j = 0; j <= 7; j++) {
    ...
    if (
      nextDayHours &&
      typeof nextDayHours === "string" &&
      nextDayHours.toLowerCase() !== "closed"
    ) {
      const normalizedHours = nextDayHours
        .replace(/\u2013|\u2011/g, "-")
        .replace(/\s+/g, " ")
        .trim();
      const [nextOpenTime, nextCloseTime] = normalizedHours
        .split("-")
        .map((time) => time.trim());
      ...
      if (j === 0) {
        ...
        return {
          statusParts: [
            { text: "Closed now", color: "#E6725A" },
            {
              text: `, opens at ${nextOpenTime} on ${nextDay}`,
              color: "#000000",
            },
          ],
        };
  ...
  return {
    statusParts: [{ text: "Closed for the week", color: "#E6725A" }],
  };
}
...
function getCurrentOccupancy(
  crowdednessMap: Record<
    string,
    Record<string, { crowdedness: number | string }>
  >,
  time: Date
): number {
  const daysOfWeek = [
  ...
  const hasHalfHourSlots = Object.keys(todayMap).some((key) =>
    key.includes(":30")
  );
  ...
  if (hasHalfHourSlots) {
    ...
    key =
      minute < 30 ? `${hourIn12Format}${amPm}` : `${hourIn12Format}:30${amPm}`;
  } else {
...
const ReviewScreen = ({
  ...
  const [openStatus, setOpenStatus] = useState<{
    statusParts: { text: string; color: string }[];
  }>({
    statusParts: [],
  });
  ...
  const [tabLayouts, setTabLayouts] = useState<{
    [key in "Menu" | "Info" | "Reviews"]?: { x: number; width: number };
  }>({});

  const fetchCafeInfo = async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/cafes/getByID/${cafeID}`
      );
  ...
  const fetchReviews = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/review/getAllbyCafe/${cafeID}`
      );
      const data = await response.json();
      setReviews(data || []);
    } catch (error) {
            ...
            <View style={styles.imageAndInfoContainer}>
              ...
              <View style={styles.cafeInfo}>
                ...
                <View style={styles.rating}>
                  <StarRating
                    rating={parseFloat(cafe.ratings["overall"]["rating"]) || 0}
                    size={20}
                    readOnly
                  />
                </View>
                <Text style={styles.cafeAddress}>
                  {`$ | ${
                    openHours
                      ? `Open from ${openHours.earliest} to ${openHours.latest}`
                      : "No hours available"
                  }`}
                </Text>
              </View>
          ...
          <View style={{ width: "100%", position: "relative" }}>
            ...
            <View style={styles.reviewContentBox}>
              <View style={styles.locationRow}>
                ...
                <Text style={styles.locationText}>{cafe.address}</Text>
              </View>
                        ...
                        style={[
                          {
                            ...
                            fontSize: 12,
                            fontStyle: "normal",
                            lineHeight: 22, // space between icon and text
                          },
                        ]}
                      >
                        {part.text}
              ...
              <View style={styles.locationRow}>
                ...
                {cafe.website ? (
                  <Text
                    style={[
                      styles.locationText,
                      { color: "#3C751E", textDecorationLine: "underline" },
                    ]}
                  ...
                  <Text style={styles.locationText}>No website available</Text>
                )}
              </View>
              ...
              <View style={styles.separator}></View>
              {
                <View>
                  ...
                  <View style={styles.labelRow}>
                    {[
                      ...Object.entries(goodLabels).map(([label, count]) => ({
                        label,
                        count,
                        color: "#3C751E",
                      })),
                      ...Object.entries(badLabels).map(([label, count]) => ({
                        label,
                        count,
                        color: "#E6725A",
                      })),
                    ].map(({ label, count, color }) => (
                      <Labels key={label} label={label} color={color} />
                    ))}
                  </View>
            ...
            <View style={styles.reviewContentBox}>
              <View style={styles.menuRow}>
                {cafe.website ? (
                  ...
                  <Text style={styles.menuWebsite}>No website available</Text>
                )}
                <WebIcon width={24} height={24} />
              </View>
              <View style={styles.separator}></View>
              <View style={styles.menuRow}>
                ...
                <Pressable onPress={() => {}}>
                  <Text style={styles.menuButton}>add picture</Text>
                </Pressable>
              </View>
              ...
              {loading ? (
                ...
                reviews
                  ...
                  .map((review, index) => {
                    ...
                    const fullComment =
                      review.cafeComments || review.caption || "";
                    const shortComment =
                        ...
                        >
                          ...
                          {isExpanded && (
                            <View style={styles.expandedContent}>
                              ...
                              {(review.labels?.good?.length ||
                                review.labels?.bad?.length) > 0 && (
                                <View
                                  style={{
                                    marginTop: 8,
                                    flexWrap: "wrap",
